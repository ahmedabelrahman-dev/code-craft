...
export const saveExecution = mutation({
  ...
  handler: async (ctx, args) => {
    console.log("saveExecution called with args:", { 
    ...
    console.log("Identity:", identity ? { subject: identity.subject } : "null");
    
    if (!identity) {
      console.log("Not authenticated - skipping save");
      // Silently fail if not authenticated - execution can still run without saving
      return;
    }
    ...
    console.log("User found:", user ? { userId: user.userId, isPro: user.isPro } : "null");

    if (!user?.isPro && args.language !== "javascript") {
      console.log("Pro subscription required for language:", args.language);
    ...
    console.log("Execution saved successfully with ID:", executionId);
    return executionId;
  },
});
